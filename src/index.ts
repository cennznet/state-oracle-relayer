...
async function main() {
	...
	let cennznetSigner = new Keyring({ type: "sr25519" }).addFromSeed(
		cennznetSeed
	);
	console.log(
		`\nstate 🔮 relayer\n` +
	...
	await api.query.ethStateOracle.nextRequestId(
		async (nextRequestId: number) => {
			if (nextRequestId == 0) return;
			let requestId = nextRequestId - 1;
			let requestInfo = (await api.query.ethStateOracle.requests(
				requestId
			)) as any;
			if (requestInfo.isNone) return;

			let requestInputData = (await api.query.ethStateOracle.requestInputData(
				requestId
			)) as any;
			console.log(
			...
			let { returnData, blockNumber } = await ethCall(
				requestInfo.unwrap().destination,
				requestInputData
			);
